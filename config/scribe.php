...
// Scribe is a dev-only dependency. Return empty config when not installed
// to prevent "Class not found" errors in production/worker containers.
...
return [
    ...
    'description' => 'REST API для TaskMate — системы управления задачами для автосалонов.',

    // Text to place in the "Introduction" section, right after the `description`. Markdown and HTML are supported.
    'intro_text' => <<<'INTRO'
            API для управления задачами, сменами, автосалонами и пользователями.

            Все даты передаются в формате ISO 8601 UTC (`2024-01-15T10:30:00Z`).

            Аутентификация через Laravel Sanctum (Bearer token).
        INTRO,
    ...
    'auth' => [
        ...
        'extra_info' => 'Получите токен через `POST /api/v1/auth/login`. Передавайте в заголовке: `Authorization: Bearer {token}`.',
    ],
