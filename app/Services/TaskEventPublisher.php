...
class TaskEventPublisher
{
    ...
    /**
     * Опубликовать событие запроса на делегирование.
     * Критическое уведомление — всегда отправляется.
     * Отправляется целевому сотруднику.
     */
    public static function publishDelegationRequested(TaskDelegation $delegation): void
    {
        $delegation->loadMissing(['task', 'fromUser']);

        self::publish([
            'event' => 'task.delegation_requested',
            'task' => self::serializeTask($delegation->task),
            'user_ids' => [$delegation->to_user_id],
            'from_user' => $delegation->fromUser->full_name ?? 'Сотрудник',
    ...
    /**
     * Опубликовать событие принятия делегирования.
     * Критическое уведомление — всегда отправляется.
     * Отправляется инициатору и менеджерам/владельцам автосалона.
     */
    public static function publishDelegationAccepted(TaskDelegation $delegation): void
    {
        $delegation->loadMissing(['task', 'fromUser', 'toUser']);
        $task = $delegation->task;

        // Инициатор + менеджеры/владельцы автосалона
        $userIds = [$delegation->from_user_id];
        ...
        self::publish([
            ...
            'user_ids' => array_values($userIds),
            'from_user' => $delegation->fromUser->full_name ?? 'Сотрудник',
            'to_user' => $delegation->toUser->full_name ?? 'Сотрудник',
            'delegation_id' => $delegation->id,
            'timestamp' => now()->toIso8601ZuluString(),
        ]);
    }
    ...
    /**
     * Опубликовать событие отклонения делегирования.
     * Критическое уведомление — всегда отправляется.
     * Отправляется инициатору.
     */
    public static function publishDelegationRejected(TaskDelegation $delegation): void
    {
        $delegation->loadMissing(['task', 'toUser']);

        self::publish([
            'event' => 'task.delegation_rejected',
            'task' => self::serializeTask($delegation->task),
            'user_ids' => [$delegation->from_user_id],
            'to_user' => $delegation->toUser->full_name ?? 'Сотрудник',
            'reason' => $delegation->rejection_reason,
            'delegation_id' => $delegation->id,
            'timestamp' => now()->toIso8601ZuluString(),
        ]);
    }
